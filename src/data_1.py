...
class SimBEV(torch.utils.data.Dataset):
    def __init__(self, nusc, is_train, data_aug_conf, grid_conf):
        ...
        self.fix_nuscenes_formatting() # TODO
    ...
    def get_scenes(self):
        ...
        if not p.exists():
            raise FileNotFoundError(f"Cannot find samples file: tried\n"
    ...
    def get_binimg(self, rec):
        ...
        for tok in rec['anns']:
            ...
            if not inst['category_name'].split('.')[0] == 'vehicle':
                continue
            box = Box(inst['translation'], inst['size'], Quaternion(inst['rotation'])) # TODO
            box.translate(trans)
            box.rotate(rot)
    ...
    def __str__(self):
        return f"""SimBEV: {len(self)} samples. Split: {"train" if self.is_train else "val"}.
...
def compile_data(version, dataroot, data_aug_conf, grid_conf, bsz,
    ...
    nusc = NuScenes(version='v1.0-{}'.format(version), # TODO
